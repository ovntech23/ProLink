...
export const JobBoard = () => {
  ...
  const JobCard = ({ shipment }: { shipment: Shipment }) => (
        ...
        <div className="flex gap-2">
          {shipment.status !== 'delivered' && (
            <>
              ...
              <Button onClick={() => handleStatusUpdate(shipment.id, shipment.status)} size="sm">
      ...
      {reportingIncidentId === shipment.id && (
        <div className="mt-4 p-4 bg-amber-50 rounded-lg border border-amber-100 animate-in fade-in slide-in-from-top-2">
          ...
          <Button
            ...
            size="sm"
            onClick={() => handleReportIncident(shipment.id)}
          >
      ...
      <div className="relative z-10">
        ...
        <div className="grid lg:grid-cols-2 gap-8">
          <div>
            ...
            {activeJobs.length > 0 ? (
              activeJobs.map(job => <JobCard key={job.id} shipment={job} />)
            ) : (
          ...
          <div>
            ...
            {completedJobs.slice(0, 3).map(job => <JobCard key={job.id} shipment={job} />)}
          </div>
        </div>
      </div>
    </div>
