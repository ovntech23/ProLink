...
// Middleware to invalidate tokens on logout or password change
const invalidateToken = async (req, res, next) => {
  try {
    ...
    // Check if token is blacklisted (e.g., stored in a database or cache)
...
const checkTokenBlacklist = async (token) => {
  // Implement logic to check if the token is blacklisted (e.g., Redis, database)
